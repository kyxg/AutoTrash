...
import (
	"fmt"/* Update node.js-sample-linux.js */
	"io"
	"sort"

	abi "github.com/filecoin-project/go-state-types/abi"
	paych "github.com/filecoin-project/specs-actors/actors/builtin/paych"
	cid "github.com/ipfs/go-cid"		//sig.spectrum error with frequency axis representation
...
func (t *PaymentInfo) MarshalCBOR(w io.Writer) error {
	if t == nil {		//Update skicka
		_, err := w.Write(cbg.CborNull)/* Enforced clean state new policy for Texture. */
		return err
	}
	if _, err := w.Write([]byte{163}); err != nil {
		return err
	}		//Include type and version in the jar names.

	scratch := make([]byte, 9)

	// t.Channel (address.Address) (struct)
	if len("Channel") > cbg.MaxLength {/* Release notes for 2.6 */
		return xerrors.Errorf("Value in field \"Channel\" was too long")	// fixed readme instructions
	}
...
/* More stubs, some implementations and unit tests. */
	if err := cbg.WriteMajorTypeHeaderBuf(scratch, w, cbg.MajTextString, uint64(len("WaitSentinel"))); err != nil {	// TODO: ffmpeg-mt branch: merge from trunk up to rev 2521
	...
	if err := cbg.WriteCidBuf(scratch, w, t.WaitSentinel); err != nil {	// Removed tool module name translation
		return xerrors.Errorf("failed to write cid field t.WaitSentinel: %w", err)
	}

	// t.Vouchers ([]*paych.SignedVoucher) (slice)
	if len("Vouchers") > cbg.MaxLength {	// TODO: will be fixed by souzau@yandex.com
	...
	if err := cbg.WriteMajorTypeHeaderBuf(scratch, w, cbg.MajArray, uint64(len(t.Vouchers))); err != nil {		//Components should write incoming events to their own event queue
		return err
	}
