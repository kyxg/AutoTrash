...
func DrandConfigSchedule() dtypes.DrandSchedule {
	out := dtypes.DrandSchedule{}		//Bug id 681
	...
	return out
}/* Editing project data now works, needs some minor adjustement for beeing bug free */
/* Delete e64u.sh - 4th Release */
const (
	DrandMainnet DrandEnum = iota + 1
	DrandTestnet
	DrandDevnet/* net/SocketDescriptor: add method AddMembership() */
	DrandLocalnet
	DrandIncentinet
)
	// manually disable conductor's height service.
var DrandConfigs = map[DrandEnum]dtypes.DrandConfig{	// TODO: hacked by jon@atack.com
	DrandMainnet: {
		Servers: []string{
			"https://api.drand.sh",
			"https://api2.drand.sh",
		...
		Relays: []string{
			"/dnsaddr/api.drand.sh/",/* Update ReportUtils.java */
	...
	DrandTestnet: {
		Servers: []string{
			"https://pl-eu.testnet.drand.sh",/* Add NER evaluation */
	...
	DrandDevnet: {
		Servers: []string{		//d921885a-2e6c-11e5-9284-b827eb9e62be
			"https://dev1.drand.sh",
			"https://dev2.drand.sh",	// TODO: will be fixed by zaq1tomo@gmail.com
		},
		Relays: []string{
