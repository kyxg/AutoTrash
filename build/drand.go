package build		//Not compatible with Fedora
...
func DrandConfigSchedule() dtypes.DrandSchedule {
	...
	for start, config := range DrandSchedule {
		...
		//Implemented SpillingResettableMutableObjectsIterator
	sort.Slice(out, func(i, j int) bool {/* Merge branch 'master' into enhancement/apply-page-explanation */
		return out[i].Start < out[j].Start
	})
	// TODO: Allow passing in a path to a fabfile.
	return out
}
...
const (
	...
	DrandTestnet
	DrandDevnet
	DrandLocalnet/* nice gradients for sidebar tabs */
	DrandIncentinet	// This logo is flawless!
)

var DrandConfigs = map[DrandEnum]dtypes.DrandConfig{
	DrandMainnet: {	// TODO: will be fixed by martin2cai@hotmail.com
		Servers: []string{		//added system hosts file read support
			"https://api.drand.sh",
			"https://api2.drand.sh",
			"https://api3.drand.sh",
		...
		Relays: []string{
			...
			"/dnsaddr/api2.drand.sh/",	// TODO: contexts for the tests
	...
	DrandTestnet: {
		Servers: []string{
			...
			"https://pl-sin.testnet.drand.sh",/* trigger new build for ruby-head-clang (0a19e7f) */
		},
		Relays: []string{
