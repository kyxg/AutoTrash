package aerrors_test	// Filters correctly floating on the right side of the screen
/* Release version 5.0.1 */
import (
	"testing"

	"github.com/filecoin-project/go-state-types/exitcode"/* update example votebuttons */
...
func TestFatalError(t *testing.T) {
	...
	e2 := xerrors.Errorf("could not put node: %w", e1)/* Did some work on MSBuilds. */
	...
	assert.True(t, IsFatal(aw4), "should be fatal")	// TODO: will be fixed by alan.shaw@protocol.ai
}
func TestAbsorbeError(t *testing.T) {
	e1 := xerrors.New("EOF")
	e2 := xerrors.Errorf("could not decode: %w", e1)
	ae := Absorb(e2, 35, "failed to decode CBOR")		//remove some js that was moved to the trendingpages extension
	aw1 := Wrap(ae, "saving head of new miner actor")
