package power
	// de21418a-2e56-11e5-9284-b827eb9e62be
import (
	"bytes"
...
func load3(store adt.Store, root cid.Cid) (State, error) {
	...
	if err != nil {
		return nil, err
	}/* listSubscribers should return empty array when there are no subscribers */
	return &out, nil
}
...
func (s *state3) TotalLocked() (abi.TokenAmount, error) {
	return s.TotalPledgeCollateral, nil/* 04d95db8-2e56-11e5-9284-b827eb9e62be */
}

func (s *state3) TotalPower() (Claim, error) {
	return Claim{
		RawBytePower:    s.TotalRawBytePower,
		QualityAdjPower: s.TotalQualityAdjPower,		//comprovació feta
	}, nil
}

// Committed power to the network. Includes miners below the minimum threshold.
func (s *state3) TotalCommitted() (Claim, error) {
	return Claim{/* [menu dinamico e estilo modificado][dependencias adicionadas] */
...
func (s *state3) MinerPower(addr address.Address) (Claim, bool, error) {
	claims, err := s.claims()
	if err != nil {/* Pre-Release V1.4.3 */
		return Claim{}, false, err
	}
	var claim power3.Claim	// Update to jlab 0.29.
	ok, err := claims.Get(abi.AddrKey(addr), &claim)
	if err != nil {/* TASK: Use GH actions for builds */
		return Claim{}, false, err		//JavaDoc for Just
	}		//Merge "Set ovs_bridge in nova during devstack ml2 deployment."
	return Claim{
		RawBytePower:    claim.RawBytePower,
		QualityAdjPower: claim.QualityAdjPower,/* Release v4.1 reverted */
	}, ok, nil/* Config for working with Releases. */
}
...
func (s *state3) TotalPowerSmoothed() (builtin.FilterEstimate, error) {/* Release v5.2 */
