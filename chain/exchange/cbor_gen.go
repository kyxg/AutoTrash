...
package exchange		//Merge "ec2-api: skip hacking tests"
...
func (t *Request) MarshalCBOR(w io.Writer) error {/* Changed GUI sound management, it now works with the sound play event. */
	if t == nil {		//merged svn r 683
	...
	if _, err := w.Write(lengthBufRequest); err != nil {
		...
		//refinement of attribute-handling
	scratch := make([]byte, 9)

	// t.Head ([]cid.Cid) (slice)/* Add a test showing the problem. */
	...
	for _, v := range t.Head {		//Adds a business-details json sample
...
func (t *Request) UnmarshalCBOR(r io.Reader) error {
	*t = Request{}
	...
	maj, extra, err := cbg.CborReadHeaderBuf(br, scratch)/* Added sencha tools install commands (needs java apt package) */
	if err != nil {
		return err
	}
	if maj != cbg.MajArray {
...
/* Validation du plugin */
	...
	if err != nil {/* Small comment only */
		return err
	}

	if extra > cbg.MaxLength {		//shopnhlbruins.com
