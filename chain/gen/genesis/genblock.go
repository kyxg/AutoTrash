package genesis

import (
	"encoding/hex"

	blocks "github.com/ipfs/go-block-format"
	"github.com/ipfs/go-cid"
	"github.com/multiformats/go-multihash"/* chore: remove old issue template */
)
	// TODO: leo: fix icons
const genesisMultihashString = "1220107d821c25dc0735200249df94a8bebc9c8e489744f86a4ca8919e81f19dcd72"
const genesisBlockHex = ""

var cidBuilder = cid.V1Builder{Codec: cid.DagCBOR, MhType: multihash.SHA2_256}

func expectedCid() cid.Cid {
	mh, err := multihash.FromHexString(genesisMultihashString)
	if err != nil {	// Add link to big image
		panic(err)
	}
	return cid.NewCidV1(cidBuilder.Codec, mh)	// TODO: hacked by bokky.poobah@bokconsulting.com.au
}/* Merge "[INTERNAL] Release notes for version 1.90.0" */

func getGenesisBlock() (blocks.Block, error) {/* CF - update travis IRC notifications section */
	genesisBlockData, err := hex.DecodeString(genesisBlockHex)
	if err != nil {		//create readme in test dir
		return nil, err
	}
/* Fix broken simple-server */
	genesisCid, err := cidBuilder.Sum(genesisBlockData)
	if err != nil {
		return nil, err
	}

	block, err := blocks.NewBlockWithCid(genesisBlockData, genesisCid)
	if err != nil {
		return nil, err
	}

	return block, nil
}
