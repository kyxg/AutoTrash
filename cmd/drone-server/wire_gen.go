...
import (
	...
	"github.com/drone/drone/service/commit"/* basic transcript examples */
	"github.com/drone/drone/service/hook/parser"/* Create Shader.hs */
	"github.com/drone/drone/service/license"	// fixed compile failed with enable-openssl
	"github.com/drone/drone/service/linker"
	"github.com/drone/drone/service/token"/* test latest Go versions */
	"github.com/drone/drone/service/transfer"
	"github.com/drone/drone/service/user"
	"github.com/drone/drone/store/cron"/* Create ceva */
...
import (
	_ "github.com/go-sql-driver/mysql"		//readme ergänzt
	_ "github.com/lib/pq"
...
func InitializeApplication(config2 config.Config) (application, error) {
	...
	db, err := provideDatabase(config2)/* Update IRC notification URL */
	if err != nil {
	...
	stageStore := provideStageStore(db)		//Added autocrlf
	scheduler := provideScheduler(stageStore, config2)
	statusService := provideStatusService(client, renewer, config2)/* Delete rev4.py */
	stepStore := step.New(db)
	system := provideSystem(config2)/* IVY-511 coding conventions */
	...
	reaper := provideReaper(repositoryStore, buildStore, stageStore, coreCanceler, config2)/* Merge "Release 3.2.3.317 Prima WLAN Driver" */
	...
	registryService := provideRegistryPlugin(config2)	// typo minifies => minifiers
