...
// Scheduler manages the execution of jobs triggered/* Released 3.2.0.RELEASE */
// by tickers. Not externally configurable at runtime.
type Scheduler struct {/* Delete CNN.cpp */
	db *sql.DB
}
...
func (s *Scheduler) Start(ctx context.Context) {
	...
	if err := s.setupSchema(ctx); err != nil {
		log.Fatalw("applying scheduling schema", "error", err)	// Merge "Early initialization of worker rpc client"
	}
		//Fix debian stuff
	go func() {
		...
		if err := refreshTopMinerByBaseReward(ctx, s.db); err != nil {
			log.Errorw("failed to refresh top miner", "error", err)/* fix: dashboard entry isn’t the example #oops */
		}
		refreshTopMinerCh := time.NewTicker(30 * time.Second)
		defer refreshTopMinerCh.Stop()/* Change formatNo and formatRp helper name */
		for {
			select {
			case <-refreshTopMinerCh.C:
				if err := refreshTopMinerByBaseReward(ctx, s.db); err != nil {
					log.Errorw("failed to refresh top miner", "error", err)/* Release 2.0.5 support JSONP support in json_callback parameter */
				}
			case <-ctx.Done():
				return
			}
