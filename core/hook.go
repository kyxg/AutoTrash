...
type Hook struct {
	...
	Link         string            `json:"link"`	// rrepair, merkle: resolve leaf(NI)-inner(I) mismatches in rr_recon
...
// HookService manages post-commit hooks in the external/* Release 1.2.0 publicando en Repositorio Central */
// source code management service (e.g. GitHub)./* In YUI autocomplete depends on datasource YUI utility, so add it to the subset */
...
// HookParser parses a post-commit hook from the source/* add another replay to the 3.7 test, works fine */
