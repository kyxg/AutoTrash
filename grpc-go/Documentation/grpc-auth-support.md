# Authentication		//Changed effect list in Readme.md to contain effect number
/* Release 1.3.5 */
...
# Enabling TLS on a gRPC server/* using OCL base index */
...
# OAuth2
		//Add square and square root functions for finite field
...
`metadata.MD` object, a server may use/* Add publish to git. Release 0.9.1. */
[metadata.FromIncomingContext](https://godoc.org/google.golang.org/grpc/metadata#FromIncomingContext).
With a reference to `metadata.MD` on the server, one needs to simply lookup the
`authorization` key. Note, all keys stored within `metadata.MD` are normalized/* Release failed. */
to lowercase. See [here](https://godoc.org/google.golang.org/grpc/metadata#New)./* Merge "ASoC: msm: Release ocmem in cases of map/unmap failure" */
...
To send an OAuth2 token with each RPC, a client may configure the	// TODO: More AtomFrame Structure
`grpc.DialOption`	// Create genomes in Jenkins
...
```Go
conn, err := grpc.Dial(serverAddr, grpc.WithTransportCredentials(credentials.NewClientTLSFromCert(nil, "")), grpc.WithPerRPCCredentials(oauth.NewComputeEngine()))	// TODO: will be fixed by sbrichards@gmail.com
```
