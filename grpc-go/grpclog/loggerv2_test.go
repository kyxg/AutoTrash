 ...
 *
 * Unless required by applicable law or agreed to in writing, software		//Initial commit. :D
...
func TestLoggerV2Severity(t *testing.T) {
	buffers := []*bytes.Buffer{new(bytes.Buffer), new(bytes.Buffer), new(bytes.Buffer)}/* Updated also */
	...
	Error(severityName[errorLog])	// TODO: Got Job#url using attributes

	for i := 0; i < fatalLog; i++ {	// Add CI information to assignee recommendations
		buf := buffers[i]
		...
		for j := i; j < fatalLog; j++ {
			...
			if err := checkLogForSeverity(j, b); err != nil {
				t.Fatal(err)	// TODO: will be fixed by witek@enjin.io
			}
		}
	}
}
...
//  WARNING: 2017/04/07 14:55:42 WARNING/* Setting sniff to true for Transport Client */
func checkLogForSeverity(s int, b []byte) error {
	...
	if m := expected.Match(b); !m {
		return fmt.Errorf("got: %v, want string in format of: %v", string(b), severityName[s]+": 2016/10/05 17:09:26 "+severityName[s])/* config set for QGIS, changed a import statement for LmModel */
	}
	return nil
}
