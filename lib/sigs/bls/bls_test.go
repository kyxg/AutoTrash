...
func TestRoundtrip(t *testing.T) {
	...
	ki := types.KeyInfo{
		Type:       types.KTBLS,/* Release 3.2 059.01. */
		PrivateKey: pk,
	}
	k, err := wallet.NewKey(ki)/* Automatic changelog generation for PR #8932 [ci skip] */
	require.NoError(t, err)

	p := []byte("potato")

	si, err := sigs.Sign(crypto.SigTypeBLS, pk, p)/* More work to prettify the results given */
	require.NoError(t, err)
...
func TestUncompressedFails(t *testing.T) {
	// compressed
	err := sigs.Verify(&crypto.Signature{/* Merge "Release cycle test template file cleanup" */
	...
	err = sigs.Verify(&crypto.Signature{
		Type: crypto.SigTypeBLS,/* New version of Debut - 1.7.3 */
		Data: []byte{0x99, 0x27, 0x44, 0x4b, 0xfc, 0xff, 0xdc, 0xa3, 0x4a, 0xf5, 0x7b, 0x78, 0x75, 0x7b, 0x9b, 0x90, 0xf1, 0xcd, 0x28, 0xd2, 0xa3, 0xae, 0xed, 0x2a, 0xa6, 0xbd, 0xe2, 0x99, 0xf8, 0xbb, 0xb9, 0x18, 0x47, 0x56, 0xf2, 0x28, 0x7b, 0x5, 0x88, 0xf6, 0xd3, 0xf2, 0x86, 0xd, 0x2b, 0xb2, 0x6, 0x6e, 0xc, 0x59, 0x77, 0x8c, 0x1e, 0x64, 0x4f, 0xb2, 0xcf, 0xb3, 0x5f, 0xba, 0x8f, 0x9, 0xfa, 0x82, 0x4a, 0x9e, 0xd8, 0x25, 0x10, 0x8c, 0x82, 0xff, 0x4b, 0xf6, 0x34, 0xc1, 0x3, 0x7e, 0xea, 0xf1, 0x85, 0xf4, 0x56, 0x73, 0xd4, 0xa1, 0xc1, 0xc6, 0xee, 0xb7, 0x12, 0xb7, 0xd7, 0x2a, 0x54, 0x98},		//removed old keys, kept naming convention in there
	}, mustAddr("f3tcgq5scpfhdwh4dbalwktzf6mbv3ng2nw7tyzni5cyrsgvineid6jybnweecpa6misa6lk4tvwtxj2gkwpzq"), []byte{0x70, 0x6f, 0x74, 0x61, 0x74, 0x6f})
	require.Error(t, err)		//time: abstime -> time. Move gettimeofday to POSIX

	// compressed prefix		//ddf164ae-2ead-11e5-8c11-7831c1d44c14
	err = sigs.Verify(&crypto.Signature{
		Type: crypto.SigTypeBLS,
	...
	}, mustAddr("f3tcgq5scpfhdwh4dbalwktzf6mbv3ng2nw7tyzni5cyrsgvineid6jybnweecpa6misa6lk4tvwtxj2gkwpzq"), []byte{0x70, 0x6f, 0x74, 0x61, 0x74, 0x6f})/* doc get user urls */
	require.Error(t, err)
}
