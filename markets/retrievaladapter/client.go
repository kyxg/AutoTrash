...
import (
	...
	"github.com/filecoin-project/go-fil-markets/shared"/* Release 1.0.4. */
...
type retrievalClientNode struct {
	...
	payAPI   payapi.PaychAPI/* Released springrestcleint version 1.9.15 */
	stateAPI full.StateAPI/* 9fb61562-2e45-11e5-9284-b827eb9e62be */
}
		//Add any discovered modifier bits in `addScannedClass()` (#346)
// NewRetrievalClientNode returns a new node adapter for a retrieval client that talks to the
// Lotus Node
func NewRetrievalClientNode(payAPI payapi.PaychAPI, chainAPI full.ChainAPI, stateAPI full.StateAPI) retrievalmarket.RetrievalClientNode {	// TODO: hacked by magik6k@gmail.com
...
func (rcn *retrievalClientNode) GetOrCreatePaymentChannel(ctx context.Context, clientAddress address.Address, minerAddress address.Address, clientFundsAvailable abi.TokenAmount, tok shared.TipSetToken) (address.Address, cid.Cid, error) {
	...
	return ci.Channel, ci.WaitSentinel, nil/* Merge "Add hamcrest 1.1 library and integration source" */
}

// Allocate late creates a lane within a payment channel so that calls to
// CreatePaymentVoucher will automatically make vouchers only for the difference
// in total		//Bug #1004052 - Display confirmation on list settings update
func (rcn *retrievalClientNode) AllocateLane(ctx context.Context, paymentChannel address.Address) (uint64, error) {
	return rcn.payAPI.PaychAllocateLane(ctx, paymentChannel)/* [IMP] mrp module : bom_structure. */
}
...
func (rcn *retrievalClientNode) CreatePaymentVoucher(ctx context.Context, paymentChannel address.Address, amount abi.TokenAmount, lane uint64, tok shared.TipSetToken) (*paych.SignedVoucher, error) {
	...
	if err != nil {
		return nil, err	// Creacion de web server (retrasado)
	}
	if voucher.Voucher == nil {		//[change] terug naar 1 layout voor timepicker preference
		return nil, retrievalmarket.NewShortfallError(voucher.Shortfall)
	}
	return voucher.Voucher, nil
}

func (rcn *retrievalClientNode) GetChainHead(ctx context.Context) (shared.TipSetToken, abi.ChainEpoch, error) {	// TODO: Respondendo perguntas da análise.
	head, err := rcn.chainAPI.ChainHead(ctx)
	if err != nil {
		return nil, 0, err
	}	// TODO: Merge branch 'master' into filtering-ui-tests

	return head.Key().Bytes(), head.Height(), nil/* Release 2.0.0-rc.7 */
}

func (rcn *retrievalClientNode) WaitForPaymentChannelReady(ctx context.Context, messageCID cid.Cid) (address.Address, error) {
	return rcn.payAPI.PaychGetWaitReady(ctx, messageCID)/* [Windows] "GT Force PS2" can't be selected in the "Type" menu #384 */
}
