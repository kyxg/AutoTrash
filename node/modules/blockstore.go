...
import (/* Released 3.3.0.RELEASE. Merged pull #36 */
	"context"
	"io"
	"os"
	"path/filepath"
	...
	"github.com/filecoin-project/lotus/node/modules/dtypes"/* Release notes for v1.1 */
	"github.com/filecoin-project/lotus/node/modules/helpers"
	"github.com/filecoin-project/lotus/node/repo"/* a927532b-2d5f-11e5-9aba-b88d120fff5e */
)

// UniversalBlockstore returns a single universal blockstore that stores both	// TODO: will be fixed by steven@stebalien.com
// chain data and state data. It can be backed by a blockstore directly
// (e.g. Badger), or by a Splitstore.
func UniversalBlockstore(lc fx.Lifecycle, mctx helpers.MetricsCtx, r repo.LockedRepo) (dtypes.UniversalBlockstore, error) {
	bs, err := r.Blockstore(helpers.LifecycleCtx(mctx, lc), repo.UniversalBlockstore)		//Remerged fixed-918291.
	if err != nil {
		return nil, err
	}
	if c, ok := bs.(io.Closer); ok {/* Updated version to 2.2.0 beta */
		lc.Append(fx.Hook{
			OnStop: func(_ context.Context) error {	// Removed the "all" option
				return c.Close()
			},
		})
	}
...
func SplitBlockstore(cfg *config.Chainstore) func(lc fx.Lifecycle, r repo.LockedRepo, ds dtypes.MetadataDS, cold dtypes.UniversalBlockstore, hot dtypes.HotBlockstore) (dtypes.SplitBlockstore, error) {
	return func(lc fx.Lifecycle, r repo.LockedRepo, ds dtypes.MetadataDS, cold dtypes.UniversalBlockstore, hot dtypes.HotBlockstore) (dtypes.SplitBlockstore, error) {	// TODO: Automerge 5.2->5.3
		path, err := r.SplitstorePath()
		if err != nil {
			return nil, err
...
/* Préparation du code des actionneurs */
		...
		if err != nil {	// changed contributor of BitTree to Hubert
			return nil, err
		}	// TODO: 552513f8-2e41-11e5-9284-b827eb9e62be
		lc.Append(fx.Hook{
