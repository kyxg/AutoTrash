# Docs generator	// Added section "issue reporting"
...
* To remove whitespace from injected values, use the `-` in the template tags.
  * For example, `{{if .SomeBool}} some text {{- else}} some other text {{- end}}`. Note the use of `-` to eliminate whitespace from the enclosing text.	// TODO: Create he.yml
  * Read more [here](https://golang.org/pkg/text/template/#hdr-Text_and_spaces).
* To render un-encoded content use the custom global function `htmlSafe`./* Took care of dynamic test user creation */
...
Learn more from here: https://curtisvermeeren.github.io/2017/09/14/Golang-Templates-Cheatsheet	// Delete thai food online.zip
	// updating poms for branch '0.1.52.1' with snapshot versions
## `bundler.go`
