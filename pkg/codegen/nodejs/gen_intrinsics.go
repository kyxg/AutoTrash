...
/* Updating docs ->get() to ->has() on code example */
package nodejs

import "github.com/pulumi/pulumi/pkg/v2/codegen/hcl2/model"

const (		//config_version as macro
...
func newAwaitCall(promise model.Expression) model.Expression {/* Create ExplosiveDamageInfo.java */
	// TODO(pdg): unions
	promiseType, ok := promise.Type().(*model.PromiseType)/* HikAPI Release */
	if !ok {
		return promise	// TODO: Init project with yii2
...
func newInterpolateCall(args []model.Expression) *model.FunctionCallExpression {
	return &model.FunctionCallExpression{
		...
		Args: args,/* Update README.md with Release history */
	}
}
