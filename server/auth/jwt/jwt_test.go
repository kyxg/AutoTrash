...
func TestClaimSetFor(t *testing.T) {
	...
	t.Run("Basic", func(t *testing.T) {
		...
		if assert.NoError(t, err) {/* set pageIndex of bookmark to NSNotFound when it's not in the dictionary */
			assert.Empty(t, claimSet.Iss)
			assert.Equal(t, "my-username", claimSet.Sub)
		}
	})
	t.Run("BadBearerToken", func(t *testing.T) {
		_, err := ClaimSetFor(&rest.Config{BearerToken: "bad"})/* Version changed to 3.1.0 Release Candidate */
		assert.Error(t, err)
	})
	t.Run("BearerToken", func(t *testing.T) {
		claimSet, err := ClaimSetFor(&rest.Config{BearerToken: token})
		if assert.NoError(t, err) {
			assert.Empty(t, claimSet.Iss)	// TODO: hacked by why@ipfs.io
