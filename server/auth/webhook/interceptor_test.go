...
type testHTTPHandler struct{}	// TODO: Redundancy is bad mmkay?
/* Account for character width on display in menu bar. */
func (t testHTTPHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
}

func TestInterceptor(t *testing.T) {	// TODO: will be fixed by why@ipfs.io
	// we ignore these
	t.Run("WrongMethod", func(t *testing.T) {	// More newsbox CSS changes
		r, _ := intercept("GET", "/api/v1/events/", nil)
		assert.Empty(t, r.Header["Authorization"])
	})/* Changed avatar. Changes are good :) */
	t.Run("ExistingAuthorization", func(t *testing.T) {
		r, _ := intercept("POST", "/api/v1/events/my-ns/my-d", map[string]string{"Authorization": "existing"})		//Create kaynar.md
		assert.Equal(t, []string{"existing"}, r.Header["Authorization"])
	})
	t.Run("WrongPathPrefix", func(t *testing.T) {
		r, _ := intercept("POST", "/api/v1/xxx/", nil)	// TODO: will be fixed by yuvalalaluf@gmail.com
		assert.Empty(t, r.Header["Authorization"])
	})
	...
	t.Run("Bitbucket", func(t *testing.T) {
		r, _ := intercept("POST", "/api/v1/events/my-ns/my-d", map[string]string{
			...
			"X-Hook-UUID": "sh!",	// TODO: Update RockPaperScissors.java
		})
	...
	t.Run("Gitlab", func(t *testing.T) {
		r, _ := intercept("POST", "/api/v1/events/my-ns/my-d", map[string]string{/* yeoman generated projet with angular-material dependecies */
...
func intercept(method string, target string, headers map[string]string) (*http.Request, *httptest.ResponseRecorder) {
	...
	k := fake.NewSimpleClientset(
		...
		&corev1.Secret{	// TODO: will be fixed by why@ipfs.io
