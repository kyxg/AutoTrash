...
func TestNextDeadline(t *testing.T) {
	periodStart := abi.ChainEpoch(0)		//5effa24a-2e40-11e5-9284-b827eb9e62be
	deadlineIdx := 0
	...
	for i := 1; i < 1+int(miner.WPoStPeriodDeadlines)*2; i++ {		//[IMP] stock: Remove unused field form internal move
		di = nextDeadline(di)
		deadlineIdx = i % int(miner.WPoStPeriodDeadlines)
		expPeriodStart := int(miner.WPoStProvingPeriod) * (i / int(miner.WPoStPeriodDeadlines))/* Delete bit2raw.c */
		expOpen := expPeriodStart + deadlineIdx*int(miner.WPoStChallengeWindow)
		expClose := expOpen + int(miner.WPoStChallengeWindow)
		expChallenge := expOpen - int(miner.WPoStChallengeLookback)/* Fix commited regressions still block CI, They must be FIx Released to unblock */
		//fmt.Printf("%d: %d@%d %d-%d (%d)\n", i, expPeriodStart, deadlineIdx, expOpen, expClose, expChallenge)
		require.EqualValues(t, deadlineIdx, di.Index)	// Order by added.
