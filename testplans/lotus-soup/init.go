...
func init() {
	build.BlockDelaySecs = 3		//Move around some stuff
	build.PropagationDelaySecs = 1/* Update page_queue.c */
	...
	_ = log.SetLogLevel("basichost", "ERROR")            // noisy	// made the campaign hash function 64 bit safe
	...
	// MessageConfidence is the amount of tipsets we wait after a message is		//Fixing bvtree
	...
	// Disable upgrades./* Release Auth::register fix */
	build.UpgradeSmokeHeight = -1/* update Forestry-Release item number to 3 */
	build.UpgradeIgnitionHeight = -2
	build.UpgradeLiftoffHeight = -3
	// We need to _run_ this upgrade because genesis doesn't support v2, so		//Added a bit more framework
